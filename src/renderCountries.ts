...
const requestURL =
  "https://restcountries.com/v3.1/all?fullText=true&fields=name,flags,population,region,subregion,capital,currencies,languages,tld,cca3";
  ...
  modalBody.innerHTML = `
        ...
        <div class="modal-info-container">
                ...
                <div class="details-columns">
                    ...
                    <p><b>Top Level Domain: </b>${(country.tld || []).join(
                      ", "
                    )}</p>
                    <p><b>Currency: </b>${Object.values(
                      country.currencies || {}
                    )
                      .map((c) => c.name)
                      .join(", ")}</p>
            ...
            ${
              country.borders
                ...
                ${country.borders
                  .map(
                    (code) =>
                      `<button class="border-btn" data-country="${
                        countryCodeMap[code]?.official || code
                      }">${countryCodeMap[code]?.common || code}</button>`
                  )
                  .join(" ")}</p>`
                : ""
            }
